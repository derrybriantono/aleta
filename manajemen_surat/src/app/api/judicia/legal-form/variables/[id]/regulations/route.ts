...
import { linkVariableToRegulation, listRegulationsForVariable } from "@/server/modules/judicia/legal-form/regulations/jlf-regulation-service";
...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return ok({ items: await listRegulationsForVariable(db, actor, normalizeId(id)) });
  } catch (error) {
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const db = await getDatabase();
    const actor = await requireActorUser(db, await resolveActorUserId(request));
    return created(await linkVariableToRegulation(db, actor, {
      variableId: normalizeId(id),
