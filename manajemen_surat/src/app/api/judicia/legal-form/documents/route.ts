...
import { listGeneratedDocuments } from "@/server/modules/judicia/legal-form/documents/jlf-document-generation-service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const items = await listGeneratedDocuments(db, actor, {
      status: getSearchParam(request, "status"),
      templateId: getSearchParam(request, "templateId"),
      nomorPerkara: getSearchParam(request, "nomorPerkara"),
