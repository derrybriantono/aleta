...
export async function POST(request: NextRequest) {
  try {
    ...
    return ok(await runAnonymizationAssistant(db, actor, { inputText: readStringValue(payload, "inputText", { required: true, maxLength: 12000 }), context: payload.context as Record<string, unknown> | undefined }, getRequestAuditMetadata(request)));
  } catch (error) {
