  ...
  if (!assistantJudgeConfig.enabled) {
    return (
      <div className="space-y-8">
        ...
        />
        <EmptyState title="Fitur belum aktif" description="Silakan hubungi Super Admin jika Asisten Hakim perlu diaktifkan untuk role Anda." />
      </div>
    );
  }

  if (!canAccessAssistantJudge(roleId, assistantJudgeConfig)) {
    return <AccessDeniedCard />;
  }

  const enabledLinks = ASSISTANT_JUDGE_PROVIDER_ORDER.map((providerId) => ({
    providerId,
    ...assistantJudgeConfig.links[providerId],
      ...
      {enabledLinks.length > 0 ? (
        <div className="grid gap-5 md:grid-cols-3">
          {enabledLinks.map((link) => (
            <Card key={link.providerId} className="flex h-full flex-col border-border/80">
              <CardHeader>
                <div className={`mb-3 w-fit rounded-2xl border p-3 ${providerTone[link.providerId]}`}>
                  {link.providerId === "claude" ? <Scale className="h-6 w-6" /> : <Sparkles className="h-6 w-6" />}
                </div>
        ...
        <EmptyState title="Belum ada link aktif" description="Semua pilihan AI sedang dinonaktifkan. Super Admin dapat mengaktifkannya kembali dari Pengaturan Asisten Hakim." />
      )}
    </div>
  );
}
