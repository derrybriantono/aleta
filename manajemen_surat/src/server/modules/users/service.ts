...
/** Count active (non-deleted, is_active=1) super-admin accounts. */
async function countActiveSuperAdmins(db: AletaDatabase): Promise<number> {
  const row = await db.prepare(
    `SELECT COUNT(*) AS cnt FROM users
     WHERE role_id = 'super-admin' AND is_active = 1 AND deleted_at IS NULL`
  ).get<{ cnt: number }>();
...
/**
 * Resolve the final role_id when updating an existing user.
 * - Explicit "admin" / "super-admin" overrides always win.
 * - Explicit null means "Bukan Admin" → derive from position.
...
/**
 * Validate that an actor is allowed to assign a given role to a target.
 * Throws ApiError 403/409 if the action is forbidden.
 */
async function validateRoleChangeGuards(
  db: AletaDatabase,
  actor: UserPersona,
  targetUser: UserPersona | null,
  newRoleId: RoleId
): Promise<void> {
  // Admin cannot elevate anyone to Super Admin
  if (actor.roleId === "admin" && newRoleId === "super-admin") {
    throw new ApiError(403, "Admin tidak dapat membuat atau mengubah akun Super Admin.");
  }
  // Admin cannot modify a Super Admin's role at all
  if (actor.roleId === "admin" && targetUser?.roleId === "super-admin") {
    throw new ApiError(403, "Akun Super Admin hanya dapat dikelola oleh Super Admin.");
  }
  // Demoting or changing the last active Super Admin is forbidden
  if (
    targetUser?.roleId === "super-admin" &&
    newRoleId !== "super-admin" &&
    actor.roleId === "super-admin"
  ) {
    const count = await countActiveSuperAdmins(db);
    if (count <= 1) {
      throw new ApiError(409, "Tidak dapat mengubah role Super Admin terakhir yang masih aktif.");
    }
  }
}

/**
 * Validate that an actor is allowed to change the is_active status of a target.
 * Only called when newIsActive is false (blocking).
 */
async function validateBlockChangeGuards(
  db: AletaDatabase,
  actor: UserPersona,
  targetUser: UserPersona,
  newIsActive: boolean
): Promise<void> {
  if (newIsActive) return; // Re-activating is always allowed
  // Admin cannot block Super Admin
  if (actor.roleId !== "super-admin" && targetUser.roleId === "super-admin") {
    throw new ApiError(403, "Anda tidak memiliki izin untuk memblokir akun Super Admin.");
  }
  // Cannot block if it would leave no active Super Admin
  if (targetUser.roleId === "super-admin") {
    const count = await countActiveSuperAdmins(db);
    if (count <= 1) {
      throw new ApiError(409, "Tidak dapat menonaktifkan Super Admin terakhir yang masih aktif.");
    }
  }
}
...
export async function updateManagedUserInDb(
  ...
  } else if (nextRoleId !== currentUser.roleId) {
    // Implicit role change from position change — still validate
    await validateRoleChangeGuards(db, actor, currentUser, nextRoleId);
  }

  // Validate block/unblock changes
  if (canManageOthers && !isSelfUpdate && payload.isActive !== undefined) {
    await validateBlockChangeGuards(db, actor, currentUser, nextIsActive);
  }
  ...
  return withTransaction(db, async (tx) => {
    ...
    // Determine audit action based on role/block changes
    const oldRoleId = currentUser.roleId;
    const roleChanged = nextRoleId !== oldRoleId;
    const activeChanged = nextIsActive !== currentUser.isActive;
    let auditAction: string = isSelfUpdate ? "UPDATE_PROFILE" : "UPDATE_USER";
    if (!isSelfUpdate && roleChanged) {
      if (nextRoleId === "super-admin") auditAction = "PROMOTE_SUPER_ADMIN";
      else if (oldRoleId === "super-admin") auditAction = "DEMOTE_SUPER_ADMIN";
      else if (nextRoleId === "admin") auditAction = "PROMOTE_ADMIN";
      else if (oldRoleId === "admin") auditAction = "DEMOTE_ADMIN";
    } else if (!isSelfUpdate && activeChanged) {
      auditAction = nextIsActive ? "UNBLOCK_USER" : "BLOCK_USER";
    }
